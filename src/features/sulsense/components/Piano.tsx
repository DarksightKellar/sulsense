...
export function Piano({ targetNote }: PianoProps) {
  ...
  return (
    <section className="custom-scrollbar w-full overflow-x-auto rounded-xl bg-slate-900 p-4 shadow">
      <div
      ...
      >
        <div className="flex h-full w-full">
          {whiteKeys.map((key) => (
            <div
              key={key.midi}
              className={`relative h-32 flex-shrink-0 rounded-b border-r border-slate-300 transition-colors last:border-r-0 ${
                activeMidi === key.midi ? 'bg-blue-400' : 'bg-white'
              }`}
              style={{ width: `${whiteKeyWidth}px` }}
            >
              {key.name === 'C' ? (
                <div className="pointer-events-none absolute bottom-2 w-full select-none text-center text-[10px] font-medium text-slate-400">
                  C{key.oct}
                </div>
              ) : null}
            </div>
        ...
        {blackKeys.map((key) => (
          <div
            key={key.midi}
            className={`absolute top-0 z-10 h-20 rounded-b border border-slate-800 shadow-md transition-colors ${
              activeMidi === key.midi ? 'bg-blue-500' : 'bg-black'
            }`}
            style={{
